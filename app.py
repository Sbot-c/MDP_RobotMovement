...
from grid_sim import (HEADING_NAMES, LAYOUTS, TURN_OPTIONS, action_to_turn,
                      is_free, make_layout, rollout)
from grid_view import build_html
...
with st.sidebar:
    ...
    n_bins = st.slider("Bins per feature", 2, 6, 4)
...
with st.sidebar:
    ...
    if mode.startswith("Distance"):
        ...
        coll = st.number_input("Collision distance (big penalty below this)",
                               value=float(df[front_col].quantile(0.05)))
        safe = st.number_input("Safe front distance (penalty grows below this)",
                               value=float(df[front_col].quantile(0.25)))
    else:
...
if st.sidebar.button("Run Value Iteration", type="primary"):
    ...
    r_state = (wall_following_reward(means, front_col, side_col, target, coll, safe)
...
if res:
    out = res["out"]
    tab_vf, tab_sim = st.tabs(["Value function and policy", "Robot on a grid"])

    with tab_vf:
        c = st.columns(4)
        c[0].metric("States", len(out))
        c[1].metric("Iterations", len(res["deltas"]))
        c[2].metric("Final delta", f"{res['deltas'][-1]:.2e}",
                    "converged" if res["deltas"][-1] < res["tol"] else "not converged")
        ...
        if len(hit):
            row = out.iloc[hit[0]]
            st.success(f"State {int(row.state_id)}: value = {row.optimal_value:.3f}, "
                       f"best action = **{row.best_action}**")
        else:
            st.warning("This combination of bins never appears in the dataset (unseen state).")

    with tab_sim:
        st.markdown(
            "The robot is dropped into a grid room. At every step its **simulated sensors** "
            "(front / left / right / back distance) are converted into a state, the "
            "**Value Iteration policy** picks an action, and the robot moves. "
            "Press play to watch it drive.")
        feats_ = res["feats"]
        c1, c2, c3 = st.columns(3)
        layout = c1.selectbox("Room layout", LAYOUTS, index=2)
        gw = c2.slider("Grid width", 10, 30, 16)
        gh = c3.slider("Grid height", 8, 20, 11)
        cell_m = c1.slider("Cell size (metres)", 0.3, 2.0, 1.0, 0.1,
                           help="Converts grid distances to the units of your dataset (usually metres).")
        n_steps = c2.slider("Steps to simulate", 20, 600, 200, 10)
        heading = c3.selectbox("Start heading", HEADING_NAMES, index=2)
        sx = c1.slider("Start x", 1, gw - 2, 2)
        sy = c2.slider("Start y", 1, gh - 2, 1)
        walls = make_layout(layout, gw, gh)

        with st.expander("Sensor and action mapping (advanced)"):
            st.caption("Which dataset feature does each simulated sensor feed? "
                       "Features that are not fed are held at their dataset median.")
            smap = {}
            mc = st.columns(4)
            for col, sname in zip(mc, ["front", "left", "right", "back"]):
                opts = ["(none)"] + feats_
                default = next((i for i, f in enumerate(opts) if sname in f.lower()), 0)
                pick_ = col.selectbox(f"{sname} sensor", opts, index=default, key=f"sm_{sname}_{'|'.join(feats_)}")
                smap[sname] = None if pick_ == "(none)" else pick_
            st.caption("What does each action do to the robot's heading before it moves one cell?")
            turns = {}
            tc = st.columns(len(res["actions"]))
            labels = list(TURN_OPTIONS)
            for col, a in zip(tc, res["actions"]):
                dflt = list(TURN_OPTIONS.values()).index(action_to_turn(a))
                turns[a] = TURN_OPTIONS[col.selectbox(a, labels, index=dflt, key=f"turn_{a}")]

        if not is_free(walls, sx, sy):
            st.error("The start cell is inside a wall - move the start position.")
        elif not any(smap.values()):
            st.warning("Map at least one simulated sensor to a feature.")
        else:
            steps = rollout(walls, (sx, sy), HEADING_NAMES.index(heading), n_steps, cell_m, smap,
                            feats_, res["edges"], res["uniq"], res["medians"],
                            out["optimal_value"].to_numpy(), out["best_action"].tolist(), turns)
            m = st.columns(3)
            m[0].metric("Collisions", sum(s["collided"] for s in steps))
            m[1].metric("Cells visited", len({(s["x"], s["y"]) for s in steps}))
            m[2].metric("Distinct states used", len({s["state"] for s in steps}))
            html, height = build_html(walls, steps, res["actions"], cell_m)
            show_html(html, height)
            traj = pd.DataFrame([{
                "step": s["step"], "x": s["x"], "y": s["y"], "heading": HEADING_NAMES[s["heading"]],
                **{f"{k}_m": round(v, 3) for k, v in s["meters"].items()},
                "state": s["state"], "value": s["value"], "action": s["action"],
                "collided": s["collided"]} for s in steps])
            st.download_button("Download trajectory (CSV)", traj.to_csv(index=False),
                               "robot_trajectory.csv", "text/csv")
