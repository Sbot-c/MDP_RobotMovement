...
.stMarkdown p, .stMarkdown li, label p {font-family:"Bricolage Grotesque", sans-serif;}
h1, h2, h3 {font-family:"Press Start 2P", monospace !important; font-weight:400 !important; letter-spacing:0;}
h1 {font-size:1.45rem !important; line-height:1.5 !important;}
h2 {font-size:1rem !important; line-height:1.6 !important;}
h3 {font-size:.8rem !important; line-height:1.6 !important;}
div[data-testid="stMetric"] {border:4px solid #241a4b; border-radius:4px; background:#fff; padding:10px 14px; box-shadow:5px 5px 0 #241a4b;}
div[data-testid="stMetricValue"] {font-family:"Press Start 2P", monospace; font-size:1.05rem;}
.stTabs [data-baseweb="tab"] {font-family:"Press Start 2P", monospace; font-size:.62rem;}
</style>
...
with tabs[4]:
    ...
    st.dataframe(acc.style.format({"Tolerance": "{:g}", "Last change Δ": "{:.2e}", "Guaranteed within": "{:.2e}",
                                   "Actual max error": "{:.2e}"})
                 .apply(lambda r: ["background-color:#e8e2ff" if abs(r["Tolerance"] - tol) < 1e-12 else "" for _ in r], axis=1),
                 hide_index=True, use_container_width=True)
