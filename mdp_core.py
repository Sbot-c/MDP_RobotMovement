...
def wall_following_reward(means, front_col, side_col, target, collision_dist, safe_front=None):
    """State reward for wall following.

    * +1 when the wall-side distance equals `target`, falling to -1 as it drifts away.
    * A growing penalty (up to -2) once the front distance drops below `safe_front`.
    * -5 when front or side distance is below `collision_dist` (about to hit something).
    """
    side, front = means[side_col], means[front_col]
    r = (1 - (side - target).abs() / max(target, 1e-9)).clip(-1, 1)
    if safe_front:
        r = r - 2 * ((safe_front - front) / safe_front).clip(0, 1)
